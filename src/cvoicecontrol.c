...
{
    pthread_mutex_lock( &mutex_P_done );
   ...
   *****/
    ...
    pthread_mutex_unlock( &mutex_P_done );
                                         /***** give up exclusive access to audio status */
}
...
int main( int argc, char *argv[] )
{
  /***** thread variables */
         ...
         ( argc == 3 && strcmp( argv[1], "--once" ) != 0 ) )
    {
        ...
        printf
            ( "If --runonce is specified, the program will exit after the first successful\n" );
        printf
            ( "recognition run. In this case the exit code will be the id of the recognized\n" );
        printf
            ( "speaker model item. This feature is provided to allow for speech prompts in\n" );
   ...
   *****/
    if ( loadConfiguration(  ) == 0 )
    {
        ...
        fprintf( stderr,
                 "Please run microphone_config before using this application!\n" );
        exit( -1 );
    }
  ...
  /***** cleanup stuff *****/
    ...
    if ( run_once )
                /***** set exit code to the ID of the recognition result */
        exit( result_id );
...
void recognize( void )
{
   ...
   *****/
    ...
    float column_min_dist;
                         /***** minimum distance in current DTW column */
  ...
  /***** main loop of recognition thread */
	 ...
	 *****/
	   ...
	   *****/
                    for ( i = 0; i < model->total_number_of_sample_utterances;
                          i++ )
                    {
                        int I = model->direct[i]->length;
                                               /***** length of sample utterance */
                        float tmp_dist = float_max, tmp_dist2;
                                               /***** temporary variables */
      ...
      /***** loop all sample utterances */
            ...
            for ( samp = 0; samp < model->total_number_of_sample_utterances;
                  samp++ )
            {
    ...
    /***** fill actual DTW matrix column */
      ...
      /***** ccalculate the first <sloppy_corner> items in the current column */
                    ...
                    sample->matrix[0][0] =
                        2 * euklid_distance( sample->data[0], frame );
                    column_min_dist =
                        sample->matrix[0][0] / ( ( 0 + 1 ) + ( 0 + 1 ) );

                    for ( i = 1; i < sloppy_corner; i++ )
                    {
                        sample->matrix[0][i] =
                            sample->matrix[0][i - 1] +
                            euklid_distance( sample->data[i], frame );

                        tmp_dist =
                            sample->matrix[0][i] / ( ( 0 + 1 ) + ( i + 1 ) );
      ...
      /***** calculate the first <sloppy_corner+1> elements */
                    ...
                    sample->matrix[1][0] =
                        sample->matrix[0][0] + euklid_distance( sample->data[0],
                                                                frame );
                    column_min_dist =
                        sample->matrix[1][0] / ( ( 1 + 1 ) + ( 0 + 1 ) );
                    ...
                    for ( i = 2; i < sloppy_corner + 1; i++ )
                    {
                        ...
                        sample->matrix[1][i] =
                            MIN3( sample->matrix[0][i] + act_dist,
                                  ...
                                  sample->matrix[0][i - 2] +
                                  2 * euklid_distance( sample->data[i - 1],
                                                       frame ) + act_dist );

                        tmp_dist =
                            sample->matrix[1][i] / ( ( 1 + 1 ) + ( i + 1 ) );
                        if ( tmp_dist < column_min_dist )
                            column_min_dist = tmp_dist;
                    }
                }
        ...
        /***** use a simpler, smaller warping function that fits into the DTW matrix */
                        ...
                        sample->matrix[pos % 3][1] =
                            MIN3( sample->matrix[pos % 3][0] + act_dist,
                                  ...
                                  2 * euklid_distance( sample->data[1],
                                                       last_frame ) +
                                  act_dist );

                        tmp_dist =
                            sample->matrix[pos % 3][1] / ( ( pos + 1 ) +
                                                           ( 1 + 1 ) );
                        if ( tmp_dist < column_min_dist )
                            column_min_dist = tmp_dist;
                    }
	   ...
	   *****/
        ...
        /***** calculate score from upper right corner of DTW matrix */
                        ...
                        int s;
                        float tmp_score =
                            sample->matrix[pos % 3][sample->length -
                                                    1] / ( pos +
                                                           sample->length );
    ...
    /*****  else if (pos > 1) */
	   ...
	   *****/
                    ...
                    {
                        bottom =
                            MAX3( 2, pos - adjust_window_width,
                                  ( pos - 2 ) / 2 );
                        top =
...
void record( void )
{
   ...
   *****/
    ...
    int new_start = 1;
                     /***** first if statement inside while loop initializes the recording */
  ...
  /***** main loop of the audio recording thread */
    ...
    /***** prefetch data into a circular buffer ... */
                ...
                memcpy( prefetch + prefetch_pos * ( FRAG_SIZE ), buffer_raw,
                        FRAG_SIZE );
    ...
    /***** and check it for speech content */
                ...
                if ( count >= CONSECUTIVE_SPEECH_BLOCKS_THRESHOLD )
                                                      /***** if speech detected ... */
                {
                    count = 0;
      ...
      /***** ... extract the data from the prefetch buffer and insert it into queue1 */
            ...
            else if ( getAudioStatus(  ) == A_recording )
                                                /***** currently recording audio data ... */
            {
